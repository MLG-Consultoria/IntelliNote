...
const ReSkilling: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const result = await GeminiService.analyzeReskilling(profession, skills, brief);
      setAiResponse(result);
    } catch (rawErr: any) {
      const msg = (rawErr && rawErr.message) ? rawErr.message : JSON.stringify(rawErr);
      console.error("[ReSkilling] error:", rawErr);
      setError(msg || "Houve um erro ao gerar sua análise. Tente novamente.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-gray-400">Ative para respostas curtas e diretas (bom para cartões/preview)</span>
            </div>
