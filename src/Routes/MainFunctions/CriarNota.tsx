...
// Importando funções do NOVO storage (que conecta com backend)
...
const CriarNota = () => {
    ...
    const [saving, setSaving] = useState(false); // Estado para loading
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            // Chama a função adaptada do storage.ts que decide se vai pra API ou Local
            ...
            if (addToCalendar) {
                // Lembretes continuam locais por enquanto no seu código
                addReminder(titulo, conteudo, calendarDate, newNoteId);
            }

            setLastCreatedId(newNoteId);
            setTitulo('');
            setConteudo('');
            setTags([]);
        ...
        <main className="flex flex-col w-full text-gray-900 dark:text-gray-100 p-4 sm:p-8 relative">
            ...
            {showPopup && (
                <PopUpNote 
                    ...
                    onClose={() => {
                        setShowPopup(false);
                        // Se quiser redirecionar após fechar o popup, pode descomentar:
                        // if (lastCreatedId) navigate(`/editar/${encodeURIComponent(lastCreatedId)}`);
                    }}
                    // Adiciona botão de editar no popup igual a versão nova
                    onEdit={lastCreatedId ? () => navigate(`/editar/${encodeURIComponent(lastCreatedId)}`) : undefined}
                />
            )}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                ...
                <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-3 items-end sm:items-center">
                    ...
                    {/* Botão Calendário (Mantido visual antigo) */}
