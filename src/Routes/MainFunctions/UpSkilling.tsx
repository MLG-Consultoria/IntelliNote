...
const UpSkilling: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const result = await GeminiService.analyzeUpskilling(skills, brief);
      setAiResponse(result);
    } catch (rawErr: any) {
      // Extrai mensagem amigável do error
      const msg = (rawErr && rawErr.message) ? rawErr.message : JSON.stringify(rawErr);
      console.error("[UpSkilling] error:", rawErr);
      setError(msg || "Houve um erro ao conectar com a IA. Tente novamente.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <main className="flex-1 flex flex-col justify-center px-4 py-8 p-10 sm:py-10 lg:px-10 lg:py-10 xl:px-32 2xl:px-48">
      <article className="gap-1 flex flex-col">
        ...
        <p className="text-gray-400">Descubra os próximos passos para evoluir suas habilidades com ajuda da IA.</p>
      </article>
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">Resposta resumida</span>
              </label>
              <span className="text-xs text-gray-400">Ative para obter bullets curtos (útil para preview/ cartões)</span>
            </div>
