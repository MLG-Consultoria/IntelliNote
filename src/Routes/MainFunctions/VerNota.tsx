...
// Imports atualizados para o novo storage.ts
import { getUserNotes, refreshUserNotesFromApi, moveToTrash, type Note } from "../../lib/storage"; 
...
const VerNota = () => {
    ...
    useEffect(() => {
        const loadNotes = async () => {
            // Tenta carregar do cache primeiro (rápido)
            setNotes(getUserNotes());
            
            // Tenta atualizar da API (lento)
            await refreshUserNotesFromApi();
            setNotes(getUserNotes());
            setLoading(false);
        };
        loadNotes();
    ...
    const confirmDelete = async () => {
        if (noteToDelete) {
            // Usa moveToTrash em vez de deleteNote para salvar na lixeira local antes de apagar da API
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {displayedNotes.map((note) => (
                        <article key={note.id} className="group relative bg-neutral-100 dark:bg-neutral-900 border border-gray-800 hover:border-gray-700 rounded-xl p-5 transition-all hover:shadow-lg flex flex-col justify-between h-[250px]">
                            ...
                            {/* Botão de editar do novo, mas posicionado no estilo antigo */}
