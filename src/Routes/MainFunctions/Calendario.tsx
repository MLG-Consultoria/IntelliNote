// src/Routes/MainFunctions/Calendario.tsx
        ...
        <div className="bg-white dark:bg-[#111113] border border-gray-200 dark:border-gray-800 rounded-xl p-6 shadow-sm h-[500px] flex flex-col justify-center relative">
          ...
          <div className="bg-gray-50 dark:bg-[#18181b] rounded-lg p-4 border border-gray-200 dark:border-gray-800 w-full max-w-sm mx-auto mt-12 md:mt-10 md:w-full md:h-3/4">
            <div className="flex items-center justify-between mb-4 px-2">
              <button onClick={() => setCurrentDate(new Date(year, month - 1, 1))} className="p-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-gray-400 hover:text-white transition-colors"><FaChevronLeft size={14} /></button>
              <span className="font-semibold text-sm uppercase tracking-wide text-gray-700 dark:text-gray-200">{months[month]} {year}</span>
              <button onClick={() => setCurrentDate(new Date(year, month + 1, 1))} className="p-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded text-gray-400 hover:text-white transition-colors"><FaChevronRight size={14} /></button>
            </div>

            <div className="grid grid-cols-7 text-center mb-2">{daysOfWeek.map(d => <span key={d} className="text-xs text-gray-500 font-medium py-1">{d}</span>)}</div>
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {days.map(day => {
                ...
                return (
                  <button key={day} onClick={() => handleDateClick(day)} className={`
                    h-9 w-9 text-sm rounded-lg flex flex-col items-center justify-center relative transition-all
                    ${isSelectedCheck ? 'bg-neutral-900 dark:bg-neutral-100 text-neutral-100 dark:text-neutral-900 font-bold shadow-md transform scale-105' : 'text-gray-400 hover:bg-neutral-200 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'}
                    ${isTodayCheck && !isSelectedCheck ? 'text-blue-500 font-bold' : ''}
                  `}>
                    {day}
                    {hasReminder && !isSelectedCheck && <span className="absolute bottom-1 w-1 h-1 bg-blue-500 rounded-full"></span>}
                  </button>
                );
              })}
            </div>
        ...
        <div className="bg-white dark:bg-[#111113] border border-gray-200 dark:border-gray-800 rounded-xl p-6 shadow-sm flex flex-col h-[500px]">
          ...
          <div className="flex-1 overflow-y-auto pr-2 mb-4 space-y-3 custom-scrollbar">
            {dayReminders.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-gray-500 opacity-60"><p>Nenhum lembrete para esta data</p></div>
            ) : (
              dayReminders.map(reminder => (
                <div key={reminder.id} onClick={() => handleNavigateToNote(reminder.title)} className="bg-gray-50 dark:bg-[#18181b] p-4 rounded-lg border border-gray-200 dark:border-gray-800 flex justify-between items-start group hover:border-gray-400 dark:hover:border-gray-600 transition-all cursor-pointer animate-fade-in-slide-down">
                  <div className="flex flex-col gap-1 pr-4 overflow-hidden">
                    <span className="font-bold text-gray-900 dark:text-white truncate w-full">{reminder.title}</span>
                    <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2">{reminder.content}</p>
                  </div>

                  <button onClick={(e) => handleDeleteReminder(e, reminder.id)} className="text-gray-400 hover:text-red-500 p-1 transition-colors -mt-1 -mr-1" title="Remover lembrete"><FaTrash size={14} /></button>
                </div>
              ))
            )}
          </div>
