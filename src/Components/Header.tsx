...
import { FaSearch, FaPlus, FaSignOutAlt, FaBars, FaTimes, FaTrashAlt } from "react-icons/fa";
import { HiUser } from "react-icons/hi";

// MUDANÇA 1: Imports atualizados do código NOVO
...
export const Header = () => {
  ...
  // MUDANÇA 2: Lógica de pegar usuário do código NOVO
  const user = getCurrentUser();

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/VerNota?q=${encodeURIComponent(searchQuery)}`);
  ...
  const handleLogout = () => {
    // MUDANÇA 3: Função de logout do código NOVO
    clearSession();
  ...
  const navLinkClasses = "text-sm text-gray-400 hover:text-gray-700 hover:dark:hover:text-gray-500 transition-colors";
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-gray-300 dark:border-gray-700/50 bg-white dark:bg-[#111113]">
      {/* LAYOUT MANTIDO DO ANTIGO */}
      <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between gap-2 px-4 lg:px-8">

        <Link to="/MenuPrincipal" className="flex items-center gap-2 z-50 shrink-0">
          ...
          <span className="hidden xl:inline-block font-bold text-black dark:text-white">
            IntelliNote
          </span>
        </Link>

        {/* Menu Central - Mantido do ANTIGO (com Lixeira) */}
        <nav className="hidden md:flex items-center gap-4 lg:gap-6 text-sm mx-4">
          ...
          <div className="flex items-center gap-3 border-l border-gray-200 dark:border-gray-700 pl-3 ml-1">
            <Link to="/Calendario" className={navLinkClasses} title="Calendário">
                <IoCalendarClearOutline className="h-5 w-5" />
            </Link>
            
            <Link to="/Lixeira" className={`${navLinkClasses} hover:text-red-500 dark:hover:text-red-400`} title="Lixeira">
        ...
        <div className="hidden md:flex ml-auto items-center gap-2 shrink-0">
          <form onSubmit={handleSearch} className="hidden md:flex items-center gap-2">
            <div className="relative">
              ...
              <input
                type="search"
                placeholder="Buscar..."
                // Mantido larguras específicas do ANTIGO
                className="h-9 w-[140px] lg:w-[180px] xl:w-240px rounded-md border border-gray-700/50 bg-gray-100/10 pl-9 pr-3 text-sm text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500 transition-all"
                value={searchQuery}
          ...
          <Link to="/CriarNota">
            <button
              className="flex h-9 items-center justify-center gap-2 rounded-md bg-neutral-900 dark:bg-white px-3 text-sm font-medium text-gray-300 dark:text-gray-900 hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors cursor-pointer"
              title="Nova Nota"
            >
              <FaPlus className="h-3 w-3" />
              {/* Mantido breakpoint XL do ANTIGO */}
              <span className="hidden xl:inline">Nova Nota</span>
            </button>
          </Link>
          ...
          {user && (
            <div className="relative">
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-full bg-gray-700 text-gray-400 hover:text-white hover:bg-gray-600 transition-colors cursor-pointer"
              >
                <HiUser className="h-5 w-5" />
              </button>
              {isDropdownOpen && (
                <div className="absolute right-0 top-11 w-48 rounded-md border border-gray-700/50 bg-white dark:bg-[#1C1C1E] shadow-lg z-50">
                  <div className="p-1">
                    <div className="px-2 py-1.5 text-sm font-medium text-gray-900 dark:text-gray-400 opacity-70">
                      {/* MUDANÇA 4: user.name para user.nome (padrão Java) */}
                      {user.nome}
                    </div>
                    <button
                      onClick={handleLogout}
                      className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm text-left text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
                    >
        ...
        <div className="flex md:hidden ml-auto items-center gap-4">
            ...
            <button onClick={() => setIsMobileMenuOpen(true)} className="text-gray-900 dark:text-white p-2 focus:outline-none">
                <FaBars className="h-6 w-6" />
            </button>
        </div>
      </div>

      {/* Menu Mobile - Mantido ANTIGO (com Lixeira e NotesAI) */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-60 flex justify-end">
            ...
            <div className="relative h-full w-full sm:w-[350px] bg-white dark:bg-[#09090B] border-l border-gray-200 dark:border-gray-800 p-6 shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <button onClick={() => setIsMobileMenuOpen(false)} className="text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white">
                ...
                <div className="flex flex-col gap-4 flex-1 overflow-y-auto">
                    ...
                    <nav className="flex flex-col gap-2 mt-2">
                        ...
                        <div className="flex flex-col">
                            <Link to="/Calendario" onClick={closeMenu} className={`${mobileLinkClasses} flex items-center gap-2`}>
                                <IoCalendarClearOutline className="h-5 w-5" />
                                Calendário
                            </Link>
                            <Link to="/Lixeira" onClick={closeMenu} className={`${mobileLinkClasses} flex items-center gap-2 text-red-600 dark:text-red-400`}>
                                <FaTrashAlt className="h-4 w-4" />
                                Lixeira
                            </Link>
                        </div>
                        ...
                        {user && (
                            <div className="space-y-3">
                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                    {/* MUDANÇA 5: user.nome aqui também */}
                                    Logado como: <span className="font-bold text-gray-900 dark:text-white">{user.nome}</span>
                                </p>
